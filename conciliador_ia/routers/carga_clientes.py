...
@router.post("/debug-archivos")
async def debug_archivos(
    archivo_portal: UploadFile = File(...),
    archivo_xubio: UploadFile = File(...),
    archivo_cliente: Optional[UploadFile] = File(None)
):
    """
    Debug detallado de archivos subidos para identificar problemas
    """
    try:
        # Guardar archivos temporalmente
        archivos_guardados = {}
        ...
        try:
            ...
            debug_info["portal"] = {
                "filename": archivo_portal.filename,
                "columnas": list(df_portal.columns),
                "total_filas": len(df_portal),
                "muestra": df_portal.head(3).to_dict(orient="records"),
                "columnas_lower": [col.lower() for col in df_portal.columns]
            }
            
            # Información de Xubio
            debug_info["xubio"] = {
                "filename": archivo_xubio.filename,
                "columnas": list(df_xubio.columns),
                "total_filas": len(df_xubio),
                "muestra": df_xubio.head(3).to_dict(orient="records"),
                "columnas_lower": [col.lower() for col in df_xubio.columns]
            }
            
            # Información del Cliente (si existe)
            if df_cliente is not None:
                debug_info["cliente"] = {
                    "filename": archivo_cliente.filename,
                    "columnas": list(df_cliente.columns),
            ...
            # Buscar columnas en Portal
            tipo_doc_col = processor._encontrar_columna(df_portal.columns, ['tipo_doc', 'tipo_documento', 'tipo', 'ct_kind0f', 'TIPO_DOC'])
            numero_doc_col = processor._encontrar_columna(df_portal.columns, ['NUMERO_DOC', 'numero_doc', 'Numero de Documento', 'numero de documento', 'numero_documento', 'nro. doc. comprador', 'nro doc comprador', 'nro. doc comprador', 'dni', 'cuit', 'CUIT', 'NUMERO_DOC'])
            nombre_col = processor._encontrar_columna(df_portal.columns, ['nombre', 'razon_social', 'cliente', 'NOMBRE'])
            
            debug_info["mapeo_columnas"] = {
                "tipo_doc_col": tipo_doc_col,
                "numero_doc_col": numero_doc_col,
                "nombre_col": nombre_col,
                "todas_encontradas": all([tipo_doc_col, numero_doc_col, nombre_col])
            }
            
            # Test de detección de clientes
            if all([tipo_doc_col, numero_doc_col, nombre_col]):
                nuevos_clientes, errores = processor.detectar_nuevos_clientes(df_portal, df_xubio, df_cliente)
                debug_info["test_deteccion"] = {
                    "nuevos_clientes": len(nuevos_clientes),
                    "errores": len(errores),
                    "errores_detalle": errores[:5] if errores else []  # Solo primeros 5 errores
                }
            else:
                debug_info["test_deteccion"] = {
                    "error": "No se pudieron mapear todas las columnas necesarias"
                }
        ...
        logger.error(f"Error en debug_archivos: {e}")
