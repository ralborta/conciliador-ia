...
def process(ventas: pd.DataFrame, tabla_comprobantes: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    # SOLUCIÓN SIMPLE: NO separar por alícuotas - procesar TODAS las facturas juntas
    # Todas las facturas van al archivo principal sin separación
    validos = df.copy()
    
    logger.info(f"Procesando TODAS las facturas sin separación por alícuotas - Total: {len(validos)}")
    ...
    logger.info(f"Validación completada - Total válidos: {len(validos)}, Total errores: {len(errores)}")
